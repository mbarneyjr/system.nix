...
Use Domain Modelling in your grilling.
...
(Merely _reading_ `.local/CONTEXT.md` for vocabulary is not this skill: that's a one-line habit any skill can do.
...
If no `.local/CONTEXT.md` exists, create one when the first term is resolved.
...
When the user uses a term that conflicts with the existing language in `.local/CONTEXT.md`, call it out immediately.
...
### Update .local/CONTEXT.md inline

When a term is resolved, update `.local/CONTEXT.md` right there.
Don't batch these up: capture them as they happen.
Use the format in [CONTEXT-FORMAT.md](./CONTEXT-FORMAT.md).

`.local/CONTEXT.md` should be totally devoid of implementation details.
Do not treat `.local/CONTEXT.md` as a spec, a scratch pad, or a repository for implementation decisions.
